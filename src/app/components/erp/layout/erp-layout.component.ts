...
export class ErpLayoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const esStartup = this.nichoSvc.nicho === 'startup';
    const all: { id: ErpTab; label: string; svg: string }[] = [
      ...
      ...(esStartup ? [{ id: 'proyectos' as ErpTab, label: 'Proyectos', svg: '<path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>' }] : []),
    ];
    this.menuItems = all.map(item => ({ ...item, svg: this.sanitizer.bypassSecurityTrustHtml(I(item.svg)) }));
  }
