...
export class LayoutComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, private router: Router, public auth: AuthService, public theme: ThemeService, private nichoSvc: NichoService) {
    this.menuItems = [];
  }
  ...
  buildMenu() {
    ...
    const raw: { id: ModuloCRM; label: string; svg: string }[] = [
      ...
      { id: 'leads',         label: cfg.leadLabel,            svg: '<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>' },
      { id: 'oportunidades', label: cfg.oportunidadLabel,     svg: '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/>' },
      { id: 'clientes',      label: cfg.clienteLabel,         svg: '<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>' },
      { id: 'actividades',   label: cfg.actividadLabel,       svg: '<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><path d="m9 16 2 2 4-4"/>' },
    ...
    this.menuItems = raw.map(item => ({ ...item, svg: this.sanitizer.bypassSecurityTrustHtml(I(item.svg)) }));
  }
